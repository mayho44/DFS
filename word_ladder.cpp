...
#define ll long long
const int N=1e5+5;
int indx=-1,maxi=1000000,counter=0;
class Solution {
...
public:
    int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
        vector<vector<int>>adjl(wordList.size()+1);
        vector<bool>visited(wordList.size()+1,false);
          for(int i=0;i<wordList.size();i++){
            ...
            if(counter==1)adjl[0].push_back(i+1),adjl[i+1].push_back(0);
          }
        for(int i=0;i<wordList.size();i++){
